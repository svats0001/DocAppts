...
export async function createMfa() {
        ...
        try {
            ...
            const userId = cookieStore.get('userId')?.value;
            const sessionId = cookieStore.get('sessionId')?.value;
            const bookingMfaCode: BookingMfaCode = {userId: parseInt(userId!), mfaCode: result};
...
export async function checkMfaCode(mfaCodeInputVal: string) {
    try {
            const cookieStore = await cookies();
            const sessionId = cookieStore.get('sessionId')?.value;
            const userId = cookieStore.get('userId')?.value;
            const bookingMfaCode: BookingMfaCode = {userId: parseInt(userId!), mfaCode: mfaCodeInputVal};
