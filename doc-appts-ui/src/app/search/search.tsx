...
export default function Search({practices, err, remountCounter}: {practices: Practice[], err: boolean, remountCounter: number}) {
    ...
    useEffect(() => {
        ...
        if (document.getElementById('googleMapsScript') == null) {
            console.log("Search mount script");
    ...
    function handlePageLinkClick(pageNo: number) {
        ...
        if (selectedLocation in locationVals) {
            params.set('location', selectedLocation);
        }
        handlePage(pageNo, params);
    }

    async function handlePage(pageNum: number, params: URLSearchParams, searchButtonClick?: boolean) {
    ...
    function searchButtonHandler() {
        ...
        console.log(selectedLocation in locationData.locations);
        if (selectedLocation !== '' && (locationData.locations.includes(selectedLocation))) {
            params.set('location', selectedLocation);
            /*router.push("/search?location="+selectedLocation+"&page=1");*/
        } else {
            setSelectedLocation('');
            setLocationVals(locationData.locations);
            params.delete('location');
        }
        console.log("params");
        console.log(params);
        /*else {router.push("/search?page=1");}*/
        handlePage(1, params, true);
    }
    ...
    <div onClick={(e) => handleLocationSearchClick(e)}>
        ...
        <br></br><h2>Book your next healthcare appointment with DocAppts</h2><br></br><br></br>
        ...
        <ul className={styles.practiceContainer}>
             {displayedPractices.map(([practice, distance]) => (
                <li key={practice.id}><Link href={"/practice/"+practice.name+"?practice="+practice.publicId}><Card practice={practice} distance={distance}></Card></Link></li>
            ))}
        </ul> : null}
