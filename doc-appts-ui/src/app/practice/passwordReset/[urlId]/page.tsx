...
import { getPasswordResetLink, getPracticePasswordResetLink, handlePasswordEdit, handlePracticePasswordEdit, PasswordReset, PracticePasswordReset, RegisterState } from "@/app/lib/actions";
...
export default function Page() {
    ...
    pathName = pathName ? pathName.split("/").at(-1) ?? "" : "";
        useEffect(() => {
        const linkFn = async() => {
            const passwordReset: PracticePasswordReset = await getPracticePasswordResetLink(pathName!);
            if (passwordReset.urlLink) {
                Cookies.set('tmpPractice', passwordReset.practiceId?.toString() ?? '');
                setAssessLink(false);
            } else {
                setAssessLink(true);
            }
    ...
    const [errorPassword, formActionPassword, isPendingPassword] = useFormState<RegisterState, FormData>(handlePracticePasswordEdit, initialStatePassword);
