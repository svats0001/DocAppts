...
import { getCard, Card, bookAppointment, createMfa, checkMfaCode, AvailableAppointment, getPractitionerByPublicId, getPracticeByPublicId, fetchAppointmentByPublicId } from "@/app/lib/actions";
...
export default function Page(props: {params: Promise<ApptUrl>}) {
    ...
    const tmpPractitioner = await getPractitionerByPublicId(practitionerParam!);
    ...
    const tmpPractice: Practice = await getPracticeByPublicId(practiceParam!);
    setPractice(tmpPractice);
    setPracticeId0(tmpPractice.name!);
    setPracticeId1(tmpPractice.phone!);
    ...
    const tmpAppointment: AvailableAppointment = await fetchAppointmentByPublicId(appointmentParam!);
    /*const appt = params.appointmentId.split("---");
    appt[1] = appt[1].replace("%3A", ":");
    appt[2] = appt[2].replace("%3A", ":");*/
    const formattedAppointmentTimeTmp = formatStartTime(tmpAppointment.startTime!.toString());
    const formattedDateTmp: string = formatDate(tmpAppointment.date!.toString());
    ...
    const finalEndTimeForServerTmp = appt[2]+":00";
    console.log("finalDate="+finalDateForServerTmp+",startTime="+finalTimeForServerTmp+",endTime="+finalEndTimeForServerTmp);
    setFinalDateForServer(finalDateForServerTmp);
    setFinalTimeForServer(finalTimeForServerTmp);*/
    setFinalDateForServer(tmpAppointment.date!.toString())
    setFinalTimeForServer(tmpAppointment.startTime!.toString());
    setFormattedAppointmentTime(formattedAppointmentTimeTmp);
    setFormattedDate(formattedDateTmp);
    /*const fetchedAppointment: AvailableAppointment = await fetchAppointmentByDateAndTime(finalDateForServerTmp, finalTimeForServerTmp, finalEndTimeForServerTmp);*/
    setAppointmentId(tmpAppointment.id || -1);
    if (tmpPractice.billing === "No bulk billing" || (tmpPractice.billing == "Mixed" && tmpPractitioner.billing)) {
        if (tmpAppointment.id == -1) {
            setMfaCodeInputErr("Server error in fetching appointment details. Please try again later.")
        } else {
        const res = await createMfa(tmpAppointment.id || -1);
    ...
    async function resendMfaCode() {
        const res = await createMfa(appointmentId);
        if (res === "Too many codes requested in short period of time") {
            setMfaEmailLimitErr(true);
        }
    ...
    async function checkCode() {
        ...
        const result = await checkMfaCode(mfaCodeInputVal, appointmentId);
