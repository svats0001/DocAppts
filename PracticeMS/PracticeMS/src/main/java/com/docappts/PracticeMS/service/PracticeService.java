...
public class PracticeService {
    ...
    public PracticeDTO createPractice(PracticeDTO pDTO) throws Exception {
        ...
        pDTO.setPublicId(UUID.randomUUID().toString());
        PracticeDTO created = Practice.generatePracticeDTO(pRepository.saveAndFlush(PracticeDTO.generatePractice(pDTO)));
        System.out.println(created);
        return created;
    }

    public PracticeDTO getPracticeByPublicId(String publicId) throws Exception {
        Optional<Practice> pOptional = pRepository.findByPublicId(publicId);
        if (pOptional.isPresent()) {
    ...
    public PracticeDTO getPracticeByIdSession(int id, String practiceSessionId) throws Exception {
        if (sessionService.pollSession(practiceSessionId).equals("Session updated")) {
            ...
            if (pOptional.isPresent()) {
                ...
                switch (field) {
                    case "password":
                        ...
                        break;
                    case "email":
                        if (pRepository.findByEmail(practice.getEmail()).isPresent()) {
                            return "Another practice with this email already exists";
                        }
                        p.setEmail(practice.getEmail());
                        break;
                    case "name":
                    ...
                    case "address":
                        ...
                        break;
                    case "description":
                        p.setDescription(practice.getDescription());
                        break;
                    case "billing":
                        p.setBilling(practice.getBilling());
                        break;
                    case "specialty":
                        p.setSpecialty(practice.getSpecialty());
                        break;
                    case "phone":
                        p.setPhone(practice.getPhone());
                        break;
                    default:
                }
